...
library(som)
credit <- read.csv("C:/Data Science with R/Pet Projects/germancredit.csv")
credit
...
credit$purpose <- factor(credit$purpose, levels = c("A40","A41","A42","A43","A44",
...
credit[1:3,]
summary(credit)
## for illustration we consider just 3 loan characteristics: ammount, duration, installment
## Standardization of the data is preferable, especially if units of the features are quite different
...
plot(xtrain[,c("amount","duration")],col=c(4,3,6,2)[credit[train,"installment"]],
     pch=c(1,2)[as.numeric(ytrain)],main = "Predicted default, by 3 nearest neighbors",cex.main=.95)
...
legend("topleft",fill=c(4,3,6,2),legend=c(1,2,3,4),title = "installment%",horiz = TRUE,bty = "n",
       col=grey(.7),cex=.8)
legend("topleft",fill=c(4,3,6,2),legend=c(1,2,3,4),title="installment%",horiz=TRUE,bty="n",
       col=grey(.7),cex=.8)
pcorr=dim(10)
